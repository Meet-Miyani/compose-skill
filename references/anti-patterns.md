...
| Base ViewModel with scattered `updateState`/`sendEffect` | state transitions untestable without full ViewModel, mutations scattered across callbacks and coroutines | `BaseViewModel` with `updateState { copy(...) }` in 10+ places | `MviViewModel` with `handleEvent()` + pure `reduce()` — all state transitions in one function, testable without coroutines |
| Base class that forces extra logic shape | adds unnecessary ceremony | `BaseViewModel` with abstract `mapIntent`, `handleError`, `isLoading` helpers | `MviViewModel` requires `handleEvent()` + `reduce()`, nothing else |
